...
/*const validarForm = (e) => {
    ...
    switch (e.target.name) {
        case "peso":
            console.log('entre al switchhhh');
            let weight = document.getElementById('pesoImc');
            if (!weight.checkValidity() || weight == "") {
                error(weight);
                return false;
            }
            break;
        ...
        case "altura":
            let height = document.getElementById('alturaImc');
...
function error(weight) {
    document.getElementById('msjError').innerHTML = weight.validationMessage;
    weight.className = "error";
    weight.focus();
}

inputs.forEach((input) => { // por cada input, se va a ejecutar
    input.addEventListener('keyup', validarForm);  // por cada letra o numero que se ingrese, se ejecuta.
    input.addEventListener('blur', validarForm); // cuando se pierde el foco del elemento. (ej: dejo de seleccionar el campo)
});
...
function calculateImc(e) {
    ...
    if(this.querySelector('[name = peso]').value == "" || this.querySelector('[name = altura]').value == ""){
        console.log('el campo1 y 2 esta vacioooooo');
        msg.innerHTML = 'No deben quedar campos vacios';  
    }
